...
let parse_input input =
  ...
  in
  Re.all find_regex input |> List.map (fun m -> Re.Group.get m 0) |> List.map parse_instr
;;
...
let example () =
  ...
  let in2 = "xmul(2,4)&mul[3,7]!^don't()_mul(5,5)+mul(32,64](mul(11,8)undo()?mul(8,5))" in
